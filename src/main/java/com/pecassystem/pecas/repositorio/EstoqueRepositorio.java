...
public interface EstoqueRepositorio extends CrudRepository<Estoque, Integer> {
    ...
    @Query("SELECT e FROM Estoque e JOIN FETCH e.peca p JOIN FETCH e.locacao l WHERE p.partnumber LIKE %:partnumber%")
    List<Estoque> findByPartNumber(@Param("partnumber") String partnumber);

    // Busca estoques pelo nome da peça
    @Query("SELECT e FROM Estoque e JOIN FETCH e.peca p JOIN FETCH e.locacao l WHERE p.nome LIKE %:nome%")
    List<Estoque> findByNomePeca(@Param("nome") String nome);

    // Busca estoques pelo id da peça
    // Versão simplificada usando convenção de nomes
    List<Estoque> findByPecaId(int id);
